...
function createSession(input) {
  ...
  base.run(ctx,`globalThis.window={setTimeout:(callback)=>callback()};primeStatusMagicLoss=()=>null;scheduleStatusMagicSound=()=>{};queueHpAttackHistoryNotation=()=>{};addAutoLogMoveHighlight=()=>{};`);
  ...
  return {step(action=null) {
    ...
    for(const p of spawned)if(!previous.pieces.some(q=>q.id===Number(p.id))){
      if(p.type!=='pawn')throw new Error('unsupported spawned type '+p.type);
      previous.pieces.push({id:Number(p.id),owner:p.color,kind:p.type,anchor:{row:0,col:0},footprint:[],origin:null,moved:false,shielded:false,hp:null,max_hp:null,statuses:[]});
    }
    ...
    return {state:previous,actions:actions(ctx),check:base.plain(ctx,`({white:findPiecesByPredicate(p=>p.color==='white'&&isRoyalKing(p)).some(p=>isSquareAttacked(p.row,p.col,'black')),black:findPiecesByPredicate(p=>p.color==='black'&&isRoyalKing(p)).some(p=>isSquareAttacked(p.row,p.col,'white'))})`)};
  }};
}
